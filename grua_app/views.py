...
def dashboard(request):
   try:
       ...
       for solicitud in solicitudes_raw:
           ...
           fecha_servicio_corregida = solicitud.fecha_servicio - timedelta(hours=3)
...
def solicitar_asistencia(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            categoria_servicio = request.POST.get('tipo_servicio_categoria', 'asistencia')
            solicitud.tipo_servicio_categoria = categoria_servicio
            ...
            # Calcular costo según el tipo de servicio
            if categoria_servicio == 'revision':
                costo_total = 50000
                print(f"📋 Revisión técnica: Tarifa fija ${costo_total}")
            else:
                costo_total = 30000
...
def cancelar_membresia(request):
    ...
    if request.method == 'POST':
        try:
            ...
            membresia.fecha_cancelacion = get_corrected_chile_time()
...
def mercadopago_checkout(request, solicitud_id):
    ...
    try:
        ...
        preference_data = {
            "items": [
                {
                    "title": f"{'Revisión Técnica' if solicitud.tipo_servicio_categoria == 'revision' else 'Asistencia Mecánica' if solicitud.tipo_servicio_categoria == 'asistencia' else 'Servicio de Grúa'} - {solicitud.numero_orden}",
                    "description": f"{'Revisión técnica - retiro en' if solicitud.tipo_servicio_categoria == 'revision' else 'Asistencia mecánica en' if solicitud.tipo_servicio_categoria == 'asistencia' else 'Grúa desde'} {solicitud.direccion_origen[:50]}{'.' if solicitud.tipo_servicio_categoria in ['revision', 'asistencia'] else '... hasta ' + solicitud.direccion_destino[:50] + '...'}",
                    "quantity": 1,
                    "currency_id": "CLP",
    ...
    try:
        ...
        if status == 'success':
            ...
            try:
                ...
                pago.fecha_confirmacion = get_corrected_chile_time()
                if payment_id:
