...
# Sistema de emails usando Mailjet para Grúa Style
...
def send_email_mailjet(to_email, subject, html_content, text_content=None):
    """Envía email usando API de Mailjet"""
    try:
        ...
        data = {
            'Messages': [{
                "From": {
                    "Email": "contacto@www.gruastyle.com",
                    "Name": "Grúa Style"
                },
                "To": [{"Email": to_email}],
                "Subject": subject,
                "HTMLPart": html_content,
                "TextPart": text_content or "Ver versión HTML del email"
            }]
        }
        ...
        if result.status_code == 200:
            print(f"✅ Email enviado via Mailjet a: {to_email}")
            return True
        else:
            print(f"❌ Error Mailjet: {result.status_code} - {result.json()}")
            return False
            
    except Exception as e:
        print(f"❌ Error enviando email via Mailjet: {e}")
        return False
...
def format_local_datetime(dt):
    """Formatea fecha de solicitud (cuándo se creó)"""
    from django.utils import timezone
    return timezone.localtime(dt).strftime('%d/%m/%Y %H:%M')

def format_service_datetime(dt):
    """Formateo simple sin conversiones de timezone"""
    if dt:
...
def enviar_email_bienvenida(user):
    """Envía email de bienvenida HTML al usuario usando Mailjet"""
    try:
...
¡Gracias por unirte a Grúa Style!
        ...
        # Usar Mailjet
        resultado = send_email_mailjet(user.email, subject, html_content, text_content)
...
def enviar_codigo_reset_password(user):
    """Envía código de reset por email usando Mailjet"""
    try:
        # Crear código
...
def enviar_comprobante_solicitud(solicitud):
    """Envía comprobante de solicitud por email usando Mailjet"""
    try:
    ...
    <div class="email-container">
        ...
        <div class="content">
            ...
            <div class="info-section">
                ...
                <div class="info-item">
                    Fecha de Servicio: <strong>{format_service_datetime(solicitud.fecha_servicio)}</strong>
                </div>
            </div>
